...
} from '../src/database/migrations/1749898800000-SeedGuidePermissions';
...
const migration = new SeedGuidePermissions1749898800000();
...
describe('SeedGuidePermissions1749898800000 — migration behavior', () => {
  ...
  it('空数据库时 up 插入 8 个办事指南权限', async () => {
    await migration.up(qr);
    expect(store.permissions.size).toBe(8);
  ...
  it('固定 ID 内容一致时 up 跳过', async () => {
    const seedPerm = SEED_GUIDE_PERMISSIONS[0];
    ...
    await migration.up(qr);
    expect(store.permissions.size).toBe(8);
    expect(store.permissions.get(seedPerm.id)?.permissionName).toBe(seedPerm.permissionName);
  });
  ...
  it('有 role-permission 引用时 down 拒绝', async () => {
    ...
    store.rolePerms.push({
      ...
      permissionId: SEED_GUIDE_PERMISSIONS[0].id,
    });
  ...
  it('down 只删除本迁移固定 ID 数据', async () => {
    ...
    store.permissions.set(altPermId, {
      id: altPermId,
      permissionCode: 'guide:legacy:read',
    ...
    expect(store.permissions.has(SEED_GUIDE_PERMISSIONS[0].id)).toBe(false);
  });

  it('固定 ID 归属字段不一致时 down 拒绝', async () => {
    await migration.up(qr);
    const row = store.permissions.get(SEED_GUIDE_PERMISSIONS[0].id)!;
    row.permissionName = '被篡改的名称';
    await expect(migration.down(qr)).rejects.toThrow(/unexpected ownership fields/);
    expect(store.permissions.has(SEED_GUIDE_PERMISSIONS[0].id)).toBe(true);
  });

  it('固定 ID permissionCode 不匹配时 down 拒绝', async () => {
    await migration.up(qr);
    const row = store.permissions.get(SEED_GUIDE_PERMISSIONS[0].id)!;
  ...
  it('guidePermissionMatchesSeed 比较全部归属字段', () => {
    const seedPerm = SEED_GUIDE_PERMISSIONS[0];
    expect(
      guidePermissionMatchesSeed(
        {
